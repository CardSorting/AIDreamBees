...
class LocalEmbeddingEngine {
  ...
  embed(text: string): number[] {
    ...
    const tokens = trimmed.toLowerCase().split(/[^a-z0-9]+/).filter(t => t.length > 0 && !STOPWORDS.has(t));
...
export class AiService {
   ...
   */
  async evaluateLogicRelationship(_textA: string, _textB: string): Promise<'supports' | 'contradicts' | 'neutral'> {
    return 'neutral';
  }
   ...
   */
  async auditCodeAgainstRule(_path: string, _code: string, _ruleContent: string): Promise<{ violated: boolean; reason?: string }> {
  ...
  getModel(): string { return 'native-local-hashing'; }
}
