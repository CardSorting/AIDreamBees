    ...
    while (attempts < maxRetries) {
      try {
        ...
        if (attempts >= maxRetries || !isRetryable) {
          console.error(`[ActionExecutor] Task ${taskId} failed permanently after ${attempts} attempts:`, err);
          throw err;
        }

        const delay = backoffMs * Math.pow(2, attempts - 1);
        console.warn(`[ActionExecutor] Task ${taskId} retrying (${attempts}/${maxRetries}) because of retryable error: ${err.message || err}. Backoff: ${delay}ms`);
