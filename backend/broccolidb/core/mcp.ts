...
export class BroccoliDBMCP {
  ...
  private startBackgroundCleanup() {
    ...
    setInterval(() => {
      this.cleanupExpiredBranches().catch(err => console.error('[AgentGit][Lifecycle] Cleanup failed:', err));
    }, 15 * 60 * 1000);
  ...
  private async executeTool<T>(name: string, op: () => Promise<T>): Promise<{ content: { type: 'text', text: string }[], isError?: boolean }> {
    try {
  ...
  private registerTools() {
    ...
    this.server.tool(
      ...
      async (args) => {
        return this.executeTool('history', async () => {
          ...
          return history.map(h => `[${h.id.substring(0, 7)}] ${h.author}: ${h.message} (${new Date(h.timestamp).toISOString()})`).join('\n') || 'No history found matching criteria.';
        });
      }
    );
    ...
    this.server.tool(
      ...
      async (args) => {
        return this.executeTool('diff_reasoning', async () => {
          ...
          report += diff.added.map(n => ` - [${n.id.substring(0,7)}] (${n.type}) ${n.author}: ${n.message}`).join('\n') || ' None\n';
          
          report += `\n[-] Removed Hypotheses/Conclusions:\n`;
          report += diff.removed.map(n => ` - [${n.id.substring(0,7)}] (${n.type}) ${n.author}: ${n.message}`).join('\n') || ' None\n';
    ...
    this.server.tool(
      ...
      {
        branch: z.string().describe('Branch name'),
        summaryData: z.string().describe('The compacted JSON data representing the aggregated state/decisions. Must be valid JSON.'),
    ...
    this.server.tool(
      ...
      async (args) => {
        return this.executeTool('context_graph', async () => {
          ...
          return graph.map(g => `${g.path} (co-modified ${g.weight} times)`).join('\n') || 'No structural correlations found.';
        });
      }
    );
    ...
    if (this.agentContext) {
      this.server.tool(
        ...
        'Add a new node to the Knowledge Graph',
        {
          ...
          edgesJson: z.string().optional().describe('JSON string of array of GraphEdge objects: [{ targetId, type, weight }]')
        },
        async (args) => {
          return this.executeTool('add_knowledge', async () => {
            const tagsArray = args.tags ? args.tags.split(',').map(t => t.trim()).filter(Boolean) : [];
            let edgesArray = [];
            if (args.edgesJson) {
      ...
      this.server.tool(
        ...
        'Search the knowledge graph using semantic similarity (cosine) or substring matching with tag filtering',
        {
          ...
          queryEmbeddingJson: z.string().optional().describe('Optional JSON array of numbers representing the query embedding vector for cosine similarity ranking'),
          augmentWithGraph: z.boolean().optional().default(false).describe('If true, perform 1-hop traversal from top results to include neighboring context')
        },
        async (args) => {
          return this.executeTool('kb_search', async () => {
            const results = await this.agentContext!.searchKnowledge(args.query, args.tags ? args.tags.split(',').map(t => t.trim()).filter(Boolean) : undefined, args.limit, args.queryEmbeddingJson ? JSON.parse(args.queryEmbeddingJson) : undefined, { augmentWithGraph: args.augmentWithGraph });
            const formatted = results.map((r: any) => `[Node: ${r.itemId}] ${r.content}\nMetadata: ${JSON.stringify({ type: r.type, confidence: r.confidence, tags: r.tags, edges: r.edges, inboundEdges: r.inboundEdges })}`).join('\n---\n');
      ...
      this.server.tool(
        ...
        'Traverse graph edges to find interconnected knowledge nodes with directional and type/weight filtering',
        {
          ...
          direction: z.enum(['outbound', 'inbound', 'both']).optional().default('outbound').describe('Edge direction to follow'),
          edgeTypes: z.string().optional().describe('Comma-separated edge types to follow (supports,contradicts,blocks,depends_on,references)'),
      ...
      this.server.tool(
        ...
        async (args) => {
          return this.executeTool('update_knowledge', async () => {
            ...
            if (args.tags !== undefined) patch.tags = args.tags.split(',').map(t => t.trim()).filter(Boolean);
      ...
      this.server.tool(
        ...
        async (args) => {
          try {
            ...
            return { content: [{ type: 'text', text: `Successfully embedded node ${args.kbId}. Vector dimensions: ${dims}` }] };
          } catch (e: any) {
      ...
      this.server.tool(
        ...
        async (args) => {
          try {
            const tagsArray = args.tags ? args.tags.split(',').map(t => t.trim()).filter(Boolean) : undefined;
            const results = await this.agentContext!.searchKnowledge(args.query, tagsArray, args.limit, undefined, { augmentWithGraph: args.augmentWithGraph });
      ...
      this.server.tool(
        ...
        async () => {
          try {
            ...
            return { content: [{ type: 'text', text: `Batch re-embedding complete. Embedded: ${result.embeddedCount}, Skipped: ${result.skippedCount}` }] };
          } catch (e: any) {
            return { content: [{ type: 'text', text: `Error: ${e.message}` }], isError: true };
          }
        }
      ...
      this.server.tool(
        ...
        async (args) => {
          try {
            const stats = await EnvironmentTracker.getStats(this.repo.getDb(), this.repo.getBasePath(), args.agentId, args.taskId);
...
Affected Paths: ${result.affectedPaths.join(', ') || 'None'}
    ...
    this.server.tool(
      ...
      async () => {
        ...
        return { content: [{ type: 'text', text: `Telemetry queue drained. Processed ${statsBefore.pending} pending items.` }] };
      }
    );
