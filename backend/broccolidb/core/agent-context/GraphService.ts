...
export class GraphService {
	...
	async updateKnowledge(kbId: string, patch: Partial<KnowledgeBaseItem>): Promise<void> {
		...
		if (patch.metadata !== undefined) dbUpdates.metadata = JSON.stringify({ ...existing.metadata, ...patch.metadata })
	...
	async getKnowledge(itemId: string): Promise<KnowledgeBaseItem> {
		...
		if (batch.length === 0) throw new AgentGitError(`Knowledge ${itemId} not found`, "NODE_NOT_FOUND")
		return batch[0]!
	}
		...
		if (missingIds.length > 0) {
			...
			if (rows.length > 0) {
				...
				const [outboundRows, inboundRows] = await Promise.all([
					this.ctx.db.selectWhere("knowledge_edges", [{ column: "sourceId", value: foundIds, operator: "IN" }]),
					this.ctx.db.selectWhere("knowledge_edges", [{ column: "targetId", value: foundIds, operator: "IN" }]),
				])
		...
		for (let depth = 0; depth <= maxDepth; depth++) {
			...
			for (const node of nodes) {
				...
				if (depth < maxDepth) {
					...
					if (filter?.minWeight) edges = edges.filter((e) => (e.weight ?? 1.0) >= filter.minWeight!)
