...
export class ReasoningService {
	...
	async autoDiscoverRelationships(nodeId: string, limit = 5): Promise<{ discovered: number; suggestions: string[] }> {
		...
		for (const candidate of candidates) {
			...
			const relationship = await this.ctx.aiService.evaluateLogicRelationship(item.content, candidate.content)
			if (relationship !== "neutral") {
				await this.graph.updateKnowledge(nodeId, {
					edges: [...(item.edges || []), { targetId: candidate.itemId, type: relationship, weight: 0.8 }],
				})
				discoveredCount++
