...
export type { AgentBundle, AgentProfile, ImpactReport, KnowledgeBaseItem, Pedigree, ServiceContext, TraversalFilter } from "./agent-context/types.js"
import type { AgentBundle, AgentProfile, ImpactReport, KnowledgeBaseItem, Pedigree, ServiceContext, TraversalFilter } from "./agent-context/types.js"
...
export class AgentContext {
	...
	constructor(workspace: Workspace, db?: BufferedDbPool, userId?: string, _profile?: { agentId: string; name: string }) {
	...
	async annotateKnowledge(targetId: string, annotation: string, agentId?: string, metadata: Record<string, any> = {}) {
		const targetNode = await this.getKnowledge(targetId)
		const edges = [...(targetNode.edges || [])]

		const annotationId = await this.addKnowledge(`note-${crypto.randomUUID()}`, "fact", annotation, {
			tags: ["annotation"],
	...
	async addLogicalConstraint(pathPattern: string, knowledgeId: string, severity: "blocking" | "warning" = "blocking") {
	...
	async appendSharedMemory(memory: string) {
		const ws = await this._db.selectOne("workspaces", [{ column: "id", value: this._serviceContext.workspace.workspaceId }])
	...
	async getGlobalCentrality(limit?: number) {
		const rows = await this._db.selectWhere("knowledge", [{ column: "userId", value: this.userId }], undefined, {
	...
	async getAgentBundle(agentId: string): Promise<AgentBundle> {
		...
		const recent = await this._db.selectWhere("knowledge", [{ column: "userId", value: this.userId }], undefined, {
			orderBy: { column: "createdAt", direction: "desc" },
			limit: 10,
		})
		const recentKnowledge = recent.length > 0 ? await this._graphService.getKnowledgeBatch(recent.map((r) => r.id as string)) : []
		return {
			profile,
